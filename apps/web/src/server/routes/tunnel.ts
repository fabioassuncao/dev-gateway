...
import { TunnelView } from '../../shared/types.ts'
import { documentRoute } from '../openapi.ts'
import { TunnelSetupError, forgetTunnel, tunnelView } from '../core/tunnel.ts'
import { setEnvValue, readEnvFile, writeEnvFile, isWritable } from '../core/envfile.ts'
...
export function tunnelRoutes(deps: AppDeps): Hono {
  ...
  app.get('/tunnel', documentRoute({
    tag: 'Network', operationId: 'getTunnel', summary: 'Cloudflare Tunnel state and routes',
  ...
  app.post('/tunnel/setup', documentRoute({
    tag: 'Network', operationId: 'setupTunnel', summary: 'Configure the connector from a tunnel token',
    ...
    try {
      const { writeTunnelSetup } = await import('../core/tunnel.ts')
  ...
  app.post('/tunnel/enable', documentRoute({
    tag: 'Network', operationId: 'enableTunnel', summary: 'Include the connector in the running stack',
  ...
  app.post('/tunnel/disable', documentRoute({
    tag: 'Network', operationId: 'disableTunnel', summary: 'Stop including the connector',
