...
dg_defaults() {
  ...
  if [ -z "$DG_WEB_API_DOCS" ]; then
    if [ "$DEV_GATEWAY_WEB_EXPOSE" = "local" ]; then
      DG_WEB_API_DOCS=true
    else
      DG_WEB_API_DOCS=false
    fi
  fi
  ...
  export DEV_GATEWAY_PROFILE DEV_GATEWAY_PROJECT_NAME DEV_GATEWAY_NETWORK \
    ...
    DEV_GATEWAY_WEB DEV_GATEWAY_WEB_BIND_ADDRESS DEV_GATEWAY_WEB_PORT \
