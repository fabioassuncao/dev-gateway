...
const MARKER = '# dev-gateway-shares: '
...
export function renderShares(shares: StoredShare[]): string {
  ...
  for (const share of shares) {
    ...
    lines.push(`    ${router}:`)
    lines.push(`      rule: ${quote(`Host(\`${share.host}\`)`)}`)
  ...
  for (const share of shares) {
    ...
    lines.push('        servers:')
    // The container NAME, never the Compose service alias: two projects on the
    // shared network can both alias `web`, and only the name is unique.
    lines.push(`          - url: ${quote(`http://${share.container}:${share.port}`)}`)
  }
  ...
  if (protectedShares.length > 0) {
    ...
    for (const share of protectedShares) {
      lines.push(`    ${shareRouterName(share.id)}-auth:`)
      lines.push('      basicAuth:')
      lines.push('        users:')
      lines.push(`          - ${quote(`${share.user}:${share.hash}`)}`)
      lines.push('        removeHeader: true')
    }
  }
