...
help=$("$GW" --help 2>&1)
for c in bootstrap up down status doctor urls analyze init access services public dns tls remote; do
...
if ! docker info >/dev/null 2>&1; then
  ...
  for c in "status --json" "urls --json" "doctor --json" "services --json" "access list --json"; do
    it "dev-gateway $c"
